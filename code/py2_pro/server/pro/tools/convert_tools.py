...
def xml2json(xml_str=None):
    u" 将xml对象转json对象"
    import xmltodict
    json = xmltodict.parse(xml_str)
    return json

def write_json(json=None, json_path=None):
    u" 写入json文件 "
    with open(json_path, 'w') as f:
        f.wirte(str(dict(json)))

def main(xml_path=None, json_path=None):
    u' 主函数 '
    xml_str = load_xml(xml_path)
    json = xml2json(xml_str)
    if not json_path:
