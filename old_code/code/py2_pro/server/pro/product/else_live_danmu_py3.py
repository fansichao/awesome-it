...
# https://www.jianshu.com/p/346f30f176ff
python3
'''
...
def sendmsg(msgstr):
    ...
    #msgHead = (bytes(data_length), 4, 'little') \
    #          + (bytes(data_length), 4, 'little') + (bytes(code), 4, 'little')
    
    #msgHead = ''.join([str(i) for i in msgHead])    
    client.send(msgHead)
    sent = 0
    while sent < len(msg):
            ...
            with open('bullet_curtain.jl', 'a') as f:
                try:
                    for i in danmu_more:
                        ...
                        space = (20 - len(dmDict['昵称'])) * " "
                        print("[%s] 昵称:%s %s 弹幕内容:%s"%(now_time,dmDict['昵称'],space,dmDict['弹幕内容']))
                        f.write(dmJsonStr)
                        danmuNum = danmuNum + 1
                except:
                    continue
