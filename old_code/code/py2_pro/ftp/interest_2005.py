...
def interest_count(itemno,strdt,enddt):
    ...
    insert_sql = InterestCount_2005.__table__.insert(colums)
...
def interest_2005_end():
    ini_sql = """
            insert into maftp.ma_interestcount (ftp_date, id, itemno, currcd, orgno, balance, curinterest, rate, level1_itemno, dayaccum)
            select  ftp_date, id, itemno, currcd, orgno, balance, curinterest, rate, level1_itemno, dayaccum from  ma_interestcount_2005
          """

    end_sql = """truncate table ma_interestcount_2005 immediate
          """
    logger.debug(ini_sql)
    session2.execute(ini_sql)
    session2.commit()
    logger.debug(end_sql)
    session2.execute(end_sql)
    session2.commit()
