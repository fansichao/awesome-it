...
from .filster_all import fil_data
filster_deal = fil_data()
...
def interest_count(itemno,strdt,enddt):
    ...
    insert_sql = InterestCount_2003.__table__.insert(colums)
    ...
    sql = """select c.acctno as id,c.acctno,c.acctname,c.custno,c.accttypecd,org.orgno,org.orgname,dc.currcd as currcd,c.autotranflag,c.prodname,nvl(c.interestdt,'1899-12-31') interestdt,nvl(c.expiredt,'1899-12-31') as expiredt,c.unitagreementflag,
       ...
       fc.yearaccum/d.yearday as dayaccum, it.itemno,it.itemname , cu.industry,cu.industryclass,cu.risklevel ,d.date as ftp_date , fc.source_date ,c.status, it.itemtype,'调整' as rateadjustyype,c.defaulttimes as IsDefaulting ,c.enddt,  
        c.begindt as begindt , c.endamt as endamt , c.endint as endint ,c.endinttax as endinttax, (case when  (select distinct 1 from xcdw.d_s_rate where ratename='活期存款' and begindt=d.date and prodno !='non' and prodtypecd!='non') =1 then '是' else  '否' end) update_rate
           ...
           where d.date ='{0}' and left(it.itemno,4) = '{1}' and it.itemno not in ('20030103','20050202','20050204') and c.interestway !='不计息' and dc.currcd='CNY' {2} """.format(strdt,itemno,filster_deal.replace('acctno','c.acctno'))
    logger.debug(sql)
...
def interest_2003_end():
    ini_sql = """
            insert into maftp.ma_interestcount (ftp_date, id, itemno, currcd, orgno, balance, curinterest, rate, level1_itemno, dayaccum)
            select  ftp_date, id, itemno, currcd, orgno, balance, curinterest, rate, level1_itemno, dayaccum from  ma_interestcount_2003
          """

    clean_sql = """truncate table ma_interestcount_2003 immediate
          """
    logger.debug(ini_sql)
    session2.execute(ini_sql)
    session2.commit()
    logger.debug(clean_sql)
    session2.execute(clean_sql)
    session2.commit()
...
def calcute_(acctno,begindt,endt=None,special_flag=False,amnt=None):
    ...
    if not endt:
        ...
        total = yearaccum.yearaccum if yearaccum else 0
        ...
        total = total - yearaccum.yearaccum if yearaccum else 0
        ...
        logger.debug(sql %(acctno,begindt,endt))
        balance_result = xcdw_session.execute((sql %(acctno,begindt,endt))).fetchone()
